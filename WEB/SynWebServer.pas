...
uses
  SynZip, SynWebReqRes, uDataYxCisSvr, System.Variants, Winapi.ActiveX,
...
begin
  ...
  try
    ...
    FIniFile := TIniFile.Create(ExtractFilePath(ParamStr(0)) + 'YxCisSvr.ini');
...
begin
  ...
  try
    ...
    try
      if not YxCis.DoExcute(InValue, OutValue) then
        Exit;
    finally
      freeandnil(YxCis);
    end;
